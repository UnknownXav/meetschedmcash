    ...
    <div className="w-full">
      ...
      <div className="flex flex-col justify-center">
        {resp.map((val) => (
          <div
            key={val.id}
            className="bg-white shadow-md rounded-lg p-6 my-4 border border-gray-300 w-[50%] mx-auto"
          >
            ...
            <p className="text-sm">
              Payroll Status:{" "}
              {val.payrollStatus === "others"
                ? val.payrollStatus
                : val.payrollStatus}
              {/* Display the custom payroll status if "Others" is selected */}
            </p>
            <p className="text-sm">
              Client Team Members' Email Addresses joining the meeting:{" "}
              {val.clientEmails}
            </p>
            <p className="text-sm">
              RM/AM Team Members' Email Addresses joining the meeting:{" "}
              {val.rmEmails}
            </p>
          </div>
        ))}
