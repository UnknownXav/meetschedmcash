...
export default function RowItem(props: Props) {
    ...
    return (
        <TableRow key={item?.id}>
            ...
            >
                {queryId !== item?.id ? (
                    ...
                    <select
                        ...
                        style={{
                            backgroundColor:
                                ReferalStatusColors[status] || "#ccc",
                        }}
                        onChange={(e) =>
                            setStatus(e.target.value as ReferalStatusEnum)
                        }
                        value={status}
                    >
            ...
            {/* Logic for Admin User or Meeting By */}
                ...
                <TableCell className=" flex justify-end ">
                    {queryId === item?.id ? (
                        ...
                        <div className=" flex flex-row justify-end flex-1 gap-4">
                            {/* Existing Pencil (not including Enrolled Employee) */}
                            <Link href={`?id=${item?.id}`} className=" p-2 rounded-full bg-yellow-600">
                                <Pencil size={14} color={"white"} />
                            </Link>

                            {/* New Pencil for Enrolled Employee */}
                            {item?.company.isEnrolledEmployee && (
                                <Link href={`/enrolled-employee?id=${item?.id}`} className=" p-2 rounded-full bg-blue-600">
                                    <Pencil size={14} color={"white"} />
                                </Link>
                            )}
