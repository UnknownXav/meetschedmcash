...
type MeetingInfoData = z.infer<typeof meetingInfoSchema>

export default function CreateClient() {
  const [infoRoute, setInfoRoute] = useState<InfoRoute>(InfoRoute.COMPANY_INFO)
  ...
  const [meetingInfo, setMeetingInfo] = useState<MeetingInfoData>({
    meetingDate: "",
    meetingTime: "",
    clientEmail: "",
    rms: "",
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof CompanyInfoData, string>>
  >({})

  const [meetingError, setMeetingErros] = useState<
    Partial<Record<keyof MeetingInfoData, string>>
  >({})
  ...
  const handleCompanyChangeInfo = (
    ...
    setCompanyInfo((prev) => {
      ...
      const result = companyInfoSchema.safeParse(updatedForm)

      if (!result.success) {
        const issue = result.error.format()
        setErrors({
          companyName: issue.companyName?._errors?.[0] || "",
          payrollStatus: issue.payrollStatus?._errors?.[0] || "",
          payrollStatusOthers: issue.payrollStatusOthers?._errors?.[0] || "",
          contactNumber: issue.contactNumber?._errors?.[0] || "",
          contactPerson: issue.contactPerson?._errors?.[0] || "",
          email: issue.email?._errors?.[0] || "",
          estimatedNumberOfEmployee: issue?._errors?.[0] || "",
        })
      } else {
        setErrors({})
      }
  ...
  const handleChangeMeetingInfo = (
    ...
    setMeetingInfo((prev) => {
      const updateForm = { ...prev, [field]: value }

      const result = meetingInfoSchema.safeParse(updateForm)
  ...
  function handleNext() {
    ...
    setInfoRoute(InfoRoute.MEETING_INFO)
  }

  async function handleConfirm() {
    try {
      setIsLoading(true)
      const time = meetingScheduleTemplate.slice(0, 2).map((val) => val.value)
      const meetingBy = time.includes(meetingInfo.meetingTime)
        ? UserType.SPBD
        : UserType.MCASH
      ...
      if (resp) {
        ...
        setInfoRoute(InfoRoute.COMPANY_INFO)
        return
      } else {
  ...
  return (
    <div className="flex justify-center mt-10 mb-19">
      <div className="bg-white shadow-md rounded-lg p-8 w-full max-w-md">
        ...
        {infoRoute === InfoRoute.COMPANY_INFO && (
          <div className="flex flex-col gap-4 w-full">
            ...
            <Input
              ...
              onChange={(e) =>
                handleCompanyChangeInfo("companyName", e.target.value)
              }
            ...
            <Input
              ...
              onChange={(e) => handleCompanyChangeInfo("email", e.target.value)}
              error={errors.email}
            />

            <Select
              className="p-3 border border-gray-300 w-full rounded-md hover:bg-red-50"
              value={companyInfo.payrollStatus}
              onChange={(e) => {
                handleCompanyChangeInfo("payrollStatus", e.target.value)
              }}
              error={errors.payrollStatus}
            >
              {clientPayrollStatus.map((val) => {
                return (
            ...
            {companyInfo.payrollStatus.toLowerCase() === "others" && (
              <Input
                className="p-3 border border-gray-300 w-full rounded-md hover:bg-red-300"
                placeholder="Enter custom payroll status"
                value={companyInfo.payrollStatusOthers}
                onChange={(e) => {
                  handleCompanyChangeInfo("payrollStatusOthers", e.target.value)
                }}
                error={errors.payrollStatusOthers}
              />
            )}
            <Input
              className="p-3 border border-gray-300 w-full rounded-md hover:bg-red-300"
              placeholder="Number of employee"
              value={companyInfo.estimatedNumberOfEmployee}
              onChange={(e) =>
              ...
              error={errors.estimatedNumberOfEmployee}
            />
            <Input
              className="p-3 border border-gray-300 w-full rounded-md hover:bg-red-300"
              ...
              onChange={(e) =>
                handleCompanyChangeInfo("contactPerson", e.target.value)
              }
            ...
            <Input
              ...
              onChange={(e) =>
                handleCompanyChangeInfo("contactNumber", e.target.value)
              }
              error={errors.contactNumber}
            />
            <Button className="  w-fit self-end" onClick={() => handleNext()}>
              Next
            </Button>
          </div>
        )}
        {infoRoute === InfoRoute.MEETING_INFO && (
          <div className="flex flex-col gap-4">
            <h1 className=" text-lg font-semibold mt-3">Meeting Schedule</h1>
            <Input
              type="date"
              className="p-3 border border-gray-300 w-full rounded-md hover:bg-red-300"
              min="2025-01-01"
              ...
              onChange={(e) =>
                handleChangeMeetingInfo("meetingDate", e.target.value)
              }
              error={meetingError.meetingDate}
            />
            <Select
              className="p-3 border border-gray-300 w-full rounded-md hover:bg-red-300"
              value={meetingInfo.meetingTime}
              onChange={(e) =>
                handleChangeMeetingInfo("meetingTime", e.target.value)
              }
              error={meetingError.meetingTime}
            >
              <option value="">Select Meeting Time</option>
              {meetingScheduleTemplate.map((val) => {
            ...
            <Input
              ...
              value={meetingInfo.clientEmail}
              onChange={(e) =>
                handleChangeMeetingInfo("clientEmail", e.target.value)
              }
              error={meetingError.clientEmail}
            />
            ...
            <Input
              ...
              onChange={(e) => handleChangeMeetingInfo("rms", e.target.value)}
            ...
            <div className=" flex flex-row w-full justify-between">
              <Button
                ...
                onClick={() => setInfoRoute(InfoRoute.COMPANY_INFO)}
              >
                Back
              </Button>
