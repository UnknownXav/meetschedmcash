...
export async function createClient(payload:ClientDto){
    const resp = await addDoc(collection(db,FirebaseCollectionEnum.client),payload)
...
export async function getClient(){
    ...
    const clients = snapShot.docs.map((doc:QueryDocumentSnapshot<DocumentData, DocumentData>)=>{
        return {
            id:doc.id,
            ...doc.data()
        }
...
async function filterByMonth(status:MeetingStatusEnum|undefined,month:string){
    const qArr:QueryConstraint[] = [];
    if(status) qArr.push(where('meetingStatus','==',status)); 
    ...
    const clients:ClientDto[] = snapShot.docs.map((doc:QueryDocumentSnapshot<DocumentData, DocumentData>)=>{
        return {
            id:doc.id,
            ...doc.data() as Omit<ClientDto,'id'>
        }
    })
    const filterData = clients.filter(val=>getMonth(val.meetings.date).toString() === month)
...
async function filterByStatus(status:string|undefined){
    ...
    const q = query(collection(db,FirebaseCollectionEnum.client),where("meetingStatus","==",status));
...
export async function filterClientByMeetingStatus(payload:FilterClientType|undefined){
    if(!payload){
        return await getClient();
    }
